...
br = requests.get(f'https://api.lution.ee/user/balance?apikey={api}')
...
def buy_mails():
    while True:
        url = requests.get(f"https://api.lution.ee/mail/buy?apikey={api}&mailcode={type}&quantity={amount}").json()
        try:
